<?php
    ...
    $image = $dbh->query("SELECT * FROM image WHERE id = {$_GET['id']}" )->fetch(PDO::FETCH_ASSOC);
    if (isset($_GET['action']) && $_GET['action']='delete') {
        unlink('uploads/'.$image['image']);
        $dbh->exec("DELETE FROM image WHERE id = {$_GET['id']}");
        header( 'Location: /');
    }
        ...
        <div class="row">
            ...
            <div class="col-xs-12 col-sm-6 col-sm-offset-3">
                <figure class="works-item">
                    ...
                    <!-- Show uploaded image -->
                    <img src="uploads/<?=$image['image']?>" alt="[ENTRY TITLE]">
                ...
                <div>
                    <a class="btn btn-info" href="/add_edit_image.php?id=<?=$image['id']?>">Edit image</a>
                    <a class="btn btn-danger" id="delete_img" href="/view_image.php?id=<?=$image['id']?>&action=delete">Delete image</a>
                </div>
            </div>
            <!-- EO Image entry -->
        </div>
