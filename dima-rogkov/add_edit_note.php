...
if (!isset($_GET['id']) && isset($_POST['note_title']) && isset($_POST['note_content'])) {
        $sql = "SELECT COUNT(*) FROM note WHERE title = '{$_POST['note_title']}' and content = '{$_POST['note_content']}'";
        if ($res = $dbh->query($sql)) {
            if ($res->fetchColumn() == 0) {
                $sql = "INSERT INTO note (	title, content)
                    VALUES ('{$_POST['note_title']}', '{$_POST['note_content']}')";
                $dbh->exec($sql);
        }
    }
}
if (isset($_GET['id'])) {
    if (count($_POST)) {
        $sql = "UPDATE note 
                SET	title='{$_POST['note_title']}',
                content='{$_POST['note_content']}'
                  WHERE id = {$_GET['id']}";
        $dbh->exec($sql);
    }
    $sql = "SELECT * FROM note WHERE id = {$_GET['id']}";
    $note = $dbh->query($sql)->fetch(PDO::FETCH_ASSOC);
}
?>
        ...
        <nav class="navbar navbar-default">
            ...
            <div class="collapse navbar-collapse" id="main-menu">
                ...
                <ul class="nav navbar-nav navbar-right">
                    <li><a href="add_edit_image.php">Add image</a></li>
                    <li><a href="add_edit_note.php">Add note</a></li>
                </ul>
            </div><!-- /.navbar-collapse -->
            <!-- /.container-fluid -->
        </nav>
            ...
            <div class="row">
                ...
                <div class="col-xs-12">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="note_content" class="form-control" id="content" rows="5" placeholder="Entry content" required ><?= isset($note['content'])?$note['content']:''?></textarea>
                    </div>
