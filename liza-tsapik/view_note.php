<?php
    ...
    $note = $dbh->query("SELECT * FROM note WHERE id = {$_GET['id']}" )->fetch(PDO::FETCH_ASSOC);
    if (isset($_GET['action']) && $_GET['action']='delete') {
        unlink($note['content']);
        $dbh->exec("DELETE FROM note WHERE id = {$_GET['id']}");
        header( 'Location: /');
    }
?>
<!DOCTYPE html>
    ...
    <div class="container">
        ...
        <div class="row">
            ...
            <div class="col-xs-12">
                ...
                <div>
                    <a class="btn btn-info" href="/add_edit_note.php?id=<?=$note['id']?>">Edit note</a>
                    <a class="btn btn-danger" id="delete_note" href="/view_note.php?id=<?=$note['id']?>&action=delete">Delete note</a>
                </div>
            </div>
        </div>
